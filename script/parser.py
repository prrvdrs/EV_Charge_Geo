    ...
    if filename.endswith(".json"):
        ...
        with open(os.path.join(directory, filename), errors= 'ignore') as f:
            ...
            for i in data['results']:
                ...
                labeling =[]
                for g in googlePlacesType:
                    if g in typeRaw:
                        x = "1"
                    else:
                        x = "0"
                    labeling.append(x)
                column = [coordinatesCP, latCP, longCP, coordinatesPI, latPI, longPI, distanceVincenty, id, name, placeId, rating, userRating,
                          scope, vicinity, typeRaw, typeOne, typeTwo]
                for z in labeling:
                    column.append(z)
